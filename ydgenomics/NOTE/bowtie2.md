...
## Usage
```shell
# 1. 为 plant_rRNA.fa 建立索引
cd /data/work
bowtie2-build /Files/ReferenceData/Database/plant_ref_rRNA/plant_rRNA.fa plant_rRNA_index

# 2. 比对并统计（使用你的实际文件路径）
bowtie2 -x plant_rRNA_index \
  -1 /Files/SP_reads_data/0A1R_w4q20m35_N_1.fq \
  -2 /Files/SP_reads_data/0A1R_w4q20m35_N_2.fq \
  -S rRNA_mapped.sam \
  -p 8 \
  --un-conc-gz non_rRNA_%.fq.gz  # 保存未比对的reads

# 3. 查看比对统计（Bowtie2 会自动输出统计信息）
# 输出示例：
# 100000 reads; of these:
#   85000 (85.00%) were aligned to rRNA  # 这就是 rRNA 占比
#   15000 (15.00%) were unaligned
...
## references
- bowtie2 | 一种快速且节约内存的比对工具 https://mp.weixin.qq.com/s/Cgv01dhYL_H6tstquQsNJA
